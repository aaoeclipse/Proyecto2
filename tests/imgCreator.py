...
def DrawFace(nameOfFile):
    ...
    # Mouth
    mouthRandY = random.randint(0,20) + 10
    ...
    while (bottomRight[1] >= pointLeft[1] or pointLeft[1] > (centery + randomSize)):
            print(pointLeft)
            print('{} >= {}'.format(bottomRight[1] , pointLeft[1]))
            mouthRandY = random.randint(0,10) - 10
            pointLeft = (centerx - mouthSizeRand, centery - mouthRandY-2)
            pointRight = (centerx + mouthSizeRand, centery - mouthRandY+2)
    
    mouthSizeRand = random.randint(int(randomSize/6), int(randomSize/2))
    # draw.arc([pointLeft, pointRight], 0, 180 , fill='black')
    draw.arc([pointLeft, pointRight], 180, 360 , fill='black')

    img.save('Images/CarasTristes/'+nameOfFile)
